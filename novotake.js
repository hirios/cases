...
async function waitChanged(target, index = 0) {
    return new Promise(resolve => {
        ...
        let observer = new MutationObserver(function (mutations) {
            if (target.innerText.includes('*')) {
                console.log('FIELD ESTÁ OCULTO')
                return observer.observe(target, config);
            } else {
                console.log('FIELD DESOCULTADO')
                resolve(target.innerText)
                observer.disconnect();
            }
        });
...
async function setEmails(email_to, email_cc) {
    document.querySelector('[aria-label="Show CC and BCC fields"]').click()
    document.querySelectorAll('[aria-label="Remove internal user gabrielasouza@google.com from the recipient list."]').forEach((e) => e.click())
    await sleep(500)
...
async function take() {
    ...
    await waitForElemente('[aria-label="Create new email"]').then(async (botaoEmail) => {
        ...
        await waitForElemente('editor [id="email-body-content-top-content"]').then(async () => {
            print('SETANDO OS EMAILS')
            setEmails(g_clientEmail, g_accountStrategist)
        }).then(() => {
            print('TENTANDO ALTERAR O EMAIL')
